...
**Config**: `config.LANGUAGE = "en"` (or `"zh"`) — global setting for the entire simulation.

**Prompt Registry** (`agent/prompts/`):
- `en.py` — English prompt templates (DAILY_SCHEDULE, ACTION_DETAIL, FOCAL_POINTS, INSIGHTS, SYSTEM_PROMPT)
- `zh.py` — Chinese prompt templates (same 5 templates, Chinese text)
- `__init__.py` — `get_prompts(language)` returns the correct module

**Persona Files**: `data/personas/{lang}/` — each language has its own directory.
- `data/personas/en/` — English persona JSONs (descriptive fields in English)
- `data/personas/zh/` — Chinese persona JSONs (descriptive fields translated to Chinese)
- Name, address, and location keys stay in English for both languages

**LLM Client**: `generate(prompt, system_prompt=None)` — accepts optional system prompt from the registry.
...
6. **Phase 6: Simulation engine** — time loop, replay, diary ✅ (14 tests)
